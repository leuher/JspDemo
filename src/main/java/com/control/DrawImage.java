...
public class DrawImage extends HttpServlet {
     ...
     * @Return: String
     */
    private String createRandomChar(Graphics2D graphics2D,String baseChar){
     ...
     * @Return: String
     */
    private String drawRandomNum(Graphics2D graphics2D,String... createTypeFlag){
        //1.设置颜色
        ...
        // 纯大写字母
        String baseUpper = "ABCDEFGHJKLMNOPQRSTUVWXYZ";
        // 纯小写字母
        String baseLower = "abcdefghjklmnopqrstuvwxyz";
        // 所有集合
        String baseAll = baseChineseChar + baseNumLetter + baseNum + baseUpper + baseLower;
        // createTypeFlag[0]==null表示没有传递参数
        if (createTypeFlag.length > 0 && null != createTypeFlag[0]) {
           if(createTypeFlag[0].equals("ch")){
                // 截取汉字
               return  createRandomChar(graphics2D,baseChineseChar);
           }  else if (createTypeFlag[0].equals("nul")) {
                // 截取数字和大小写字母的组合
                return createRandomChar(graphics2D, baseNumLetter);
            } else if (createTypeFlag[0].equals("n")) {
                // 截取数字
                return createRandomChar(graphics2D, baseNum);
            } else if (createTypeFlag[0].equals("u")) {
                // 截取大写字母
                return createRandomChar(graphics2D, baseUpper);
            } else if (createTypeFlag[0].equals("l")){
               //  截取小写字母
                return createRandomChar(graphics2D, baseLower);
           }
        } else {
            // 默认截取数字和大小写字母的组合
            return createRandomChar(graphics2D, baseNumLetter);
